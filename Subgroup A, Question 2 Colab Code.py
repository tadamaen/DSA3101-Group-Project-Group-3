# -*- coding: utf-8 -*-
"""DSA3101 Group Project (Group 3, Subgroup A, Question 2)
...
Step 1: **Feature engineer, Data Clean and Data Preprocess** to retain the most insightful rows and scale data, in preparation for later steps.

Step 2: Apply **Principle Component Analysis(PCA)** for dimensionality reduction.

Step 3: Apply **Parameter tuning** for model, using elbow method and other visualisation methods to identify the ideal number of clusters.

Step 4: Perform **K-means clustering** model implementation based on the optimal number of clusters to label the different clusters.

Step 5: Perform **Analysis of various guest segments** and their key distinctions and characteristics.
...
Examining the dataset columns, there are several important columns and others that we deemed unnecesary as they might add variance to the model that we will implement subsequently, leading to a higher possibility of overfitting.
...
data = df[['q1', 'q2_1', 'q3',                             # Demographics
           ...
           'q19_1', 'q19_2', 'q19_3', 'q19_4',             # Importance Of Services
...
7. **Unsatisfaction Reasons from Customers:** While it is valuable for service improvement, dissatisfaction reasons are reactionary data points rather than proactive indicators of visitor segmentation. This information is better used in customer service improvements and complaint resolution strategies rather than clustering guest types.

--------------------------------------------------------------------------------

##### Data Cleaning and Preprocessing

Now that we have selected the features we intend to use for our model, we will now proceed to clean and preprocess the data. There are several steps we will use to clean and preprocess the data.
...
We can first check if there are any `NA` values for each column of the dataset. This is done using the `.isna().sum()` method on the dataframe `data`.
"""
...
"""There are several `NA` values for a few columns in the dataframe. We observe that there is 1 `NA` value for `q16_2`, 1 `NA` value for `q16_4`, 1 `NA` value for `q16_5` and 4 `NA` values for `q16_3`.

We can either remove the rows containing the `NA` values, or perform some imputation using mean, median or mode to replace them. Since there are very few rows containing `NA` values (with respect to the entire dataset of more than 500 rows), it is better to remove them.
"""
...
"""There are now 501 rows of data left, instead of the initial 505 before the 4 rows containing `NA` values are removed. Now the dataframe is free from `NA` values.

--------------------------------------------------------------------------------

###### Handling Outliers for Numerical Columns

To detect any possible outliers in the dataset, we need to check if there are any anomalies (example: extremely large and small values, negative value in time taken etc.). The values that are either irrelevant or ridiculously large/small might affect the results of the Machine Learning model.

Before checking for outliers, we need to first identify the numerical columns of the dataset. As some columns of the dataset are categorical, they exist within a fixed range/category of answers and no checking is needed.
...
However, only three columns, `q7`, `q10` and `q12`, do not have fixed categories as the survey respondants are able to type in their own responses. For the remaining questions, there are fixed choices which the users can select, but they are not able to input extra responses. This means that there are no anomilies in columns `q1`, `q2_1` and `q3`.

Therefore we only need to check for anomalies in columns `q7`, `q10` and `q12`.

Before identifying the anomalies, we need to handle the complex data for the columns `q6` and `q10`. This is because some options for `q6` and `q10` involve the use of `brackets ()` that will cause the data to be handled incorrectly if not cleaned accurately. Also, some options for `q6` and `q10` involve capital letters. To standardize, we will change all words to lowercase.

The code below performs data cleaning and text standardization on columns `q6` and `q10` in the `data_clean` dataset. First, it removes any text enclosed in parentheses (e.g., "Teacup Ride (Suspended Coasters)") using **regex** in `str.replace()` for both columns, leaving just the core ride or event name. Afterward, it converts all string values in the entire DataFrame to lowercase to ensure uniformity in text formatting, facilitating easier comparison and analysis. This ensures that all text data is cleaned and standardized before further analysis.
"""
...
"""Now, we can proced to check for possible anomalies in `q7`, `q10` and `q12` of the dataset.

We first processes each column by splitting the comma-separated string values into lists, stripping any extra spaces around the entries, then converting these lists into sets of unique values (`all_7`, `all_10`, `all_12`).

These unique value sets are compared to the original sets of predefined, valid values for each respective column and finds the differences (outliers) using the set subtraction operation. It prints the outliers for each column, helping to identify any values that do not belong to the predefined categories for better data quality and consistency.
"""
...
"""Observing for outliers, we find:
...
Similar to the `NA` rows and numerical outliers, these 3 anomalies make up a small proportion of the dataset (3 out of 499 rows, less than 1%). It is best to remove these rows containing the outliers to prevent subsequent inaccuracies in the Machine Learning Model.

We filter out rows containing these anomalous values in the columns `q7`, `q10` and `q12` from the data_clean dataset. We first define lists of anomalies for each column, then apply a lambda function to each entry in the respective columns to check if any of the anomaly values are present. The `apply()` function iterates through the entries, and the `any()` function checks if any anomaly in the list exists within the string. Using the negation operator (~), it excludes rows containing the identified anomalies by creating a boolean mask, and the filtered dataset is stored in `data_clean`, which contains only the rows that do not have the specified anomalies.
"""
...
"""We observe that after filtering out the anomaly values for the categorical variables, the number of rows of the dataset reduced by 3 from 499 rows to 496 rows, indicating that the 3 rows containing the outliers have been successfully removed. Our datset is now free of `NA`, as well as outliers for both numerical and categorical variables.

--------------------------------------------------------------------------------

###### Encoding Categorical Variables

Now our data is cleaned and we can proceed to the next step---data preprocessing. This step comprises of 2 two operations: encoding categorical variables and scaling numerical variables. We will first encode the categorical variables.
...
All the categorical variables will be encoded using **One-Hot Encoding**:

**One-Hot Encoding** is a technique used in machine learning to transform categorical variables into a format suitable for numerical algorithms. The process converts each categorical value into a binary vector where all elements are zero except for one position, which contains a one. This representation ensures that machine learning algorithms don't misinterpret categorical values as having an ordinal relationship. Although it increases dimensionality, it allows models to properly process categorical data without imposing artificial numerical relationships between categories.

The code below performs one-hot encoding on multiple categorical columns in the `data_clean` dataset (i.e. the columns listed in the categorical list).

It first uses `MultiLabelBinarizer` to encode the `q1` column by splitting comma-separated values and adding a prefix (`q1_`) to each category, resulting in binary columns for each unique category. The process is repeated for each subsequent categorical column in the list (`q2_1`, `q3`, `q6`, `q7`, `q10`, `q12`), where each column's categories are also split by commas, prefixed with the column name, and transformed into binary columns.

The resulting one-hot encoded columns are concatenated into a new DataFrame (`data_encode_cat`). The final DataFrame contains all the one-hot encoded variables from the specified categorical columns.
"""
...
We will be scaling the numerical columns using Min-Max Scaler.

**Min-max scaling**, also known as normalization, is a technique used to rescale the features of a dataset into a specified range, typically between 0 and 1. This is achieved by subtracting the minimum value of each feature and dividing by the range (the difference between the maximum and minimum values) for that feature. This process ensures all features are on the same scale, to prevent features from dominating one another. Min-max scaling is particularly useful when features have unknown distributions, different units or magnitudes. It will also help improve the convergence speed of some algorithms.

The code below scales the numerical columns of the `data_clean` dataset using min-max scaling, which normalizes each feature to a range between 0 and 1. It creates a list of numerical columns to be scaled, then selects these columns from the dataset into `data_scale_num`. A `MinMaxScaler` from the `sklearn.preprocessing` module is initialized to transform the selected data. The `fit_transform()` method is applied to scale the data, ensuring that each value in the columns is transformed based on the minimum and maximum values of the column. The scaled data is then assigned back to `data_scale_num`, and the result is printed to show the normalized numerical features.
"""
...
After encoding the categorical variables, as well as scaling the numerical variables, we can then merge the two dataframes together using the `.concat()` method.
"""
...
**K-Means clustering** is widely used for tasks like customer segmentation, anomaly detection, and pattern recognition. It is an unsupervised machine learning algorithm used to partition a dataset into a predefined number of clusters, where each cluster contains data points similar to one another. The algorithm works by first selecting a specified number of centroids (initial cluster centers) and assigning each data point to the nearest centroid. The centroids are subsequently updated by computing the mean of all data points assigned to each cluster. This process is repeated iteratively until the centroids stabilize (no longer change significantly) or the maximum number of iterations is reached.

Before implementing the K-Means Machine Learning algorithm, we need to consider using **Principal Component Analysis** in the context of this dataset.
...
"""The code above applies Principal Component Analysis (PCA) to reduce the dimensionality of the dataset `data_c` to two principal components.

First, it initializes a PCA object from `sklearn.decomposition` with `n_components=2`, specifying that the data should be transformed into two new features that capture the most variance. The `random_state=2` ensures that the results are reproducible. Next, it applies the `fit_transform()` method to the dataset, which computes the principal components and transforms the original data accordingly.

The resulting two components are stored in a new DataFrame, data_pca, with columns labeled 'pc1' and 'pc2'. This reduced data can now be used for further analysis, such as clustering, with fewer features while retaining the most significant patterns in the data.

--------------------------------------------------------------------------------

#### Step 3: Obtaining Number of Clusters for the Model
...
SSE, or Sum of Squared Errors, is a metric used to measure the total variation within a dataset not explained by the clustering model. In the context of K-Means clustering, SSE is
 an key evaluation metric because it helps quantify how well the model has fitted the data. It is calculated by summing the squared distances between each data point and its assigned cluster centroid. A lower SSE indicates that data points are closer to their centroids, and that clusters are well-defined and tightly grouped. Conversely, a higher SSE suggests points are more spread out from their centroids, indicating poorly defined clusters.
...
**Why Is Balancing SSE With Model Complexity Important?**

Balancing Sum of Squared Errors (SSE) with complexity is crucial in determining the optimal number of clusters because it ensures that the clustering model is both effective and interpretable.

1) Diminishing Returns on SSE Reduction

As the number of clusters increases, SSE always decreases because each cluster has fewer points, leading to better fit. However, after a certain point, the improvement in SSE becomes minimal compared to the additional complexity introduced by more clusters. This is known as the elbow point.

2) Overfitting vs. Underfitting

Too few clusters (underfitting) can result in high SSE, meaning that distinct groups in the data are not well-separated. Too many clusters (overfitting) can lead to low SSE but excessive complexity, making it difficult to interpret the results and generalize to new data.

--------------------------------------------------------------------------------

###### Plotting the Cluster Diagrams
...
Question 1: Which type of theme park visitor best describes each cluster?
"""
...
Question 2.1:	What is their respective age ranges?
"""
...
Question 3: Are they tourists or locals?
"""
...
Question 6: The type of attractions they enjoy most when visiting USS
"""
...
Question 7: Factors that will influence their decision to visit a theme park like USS?
"""
...
Question 10: When do they usually visit theme parks or attractions like USS?
"""
...
Question 12: How do they usually navigate a theme park like USS?
"""
...
###### Guest Segment Names and Key Traits

Having analysed the various proportions for each cluster across questions, our findings can thus be summarised as such below:
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td>0</td>
                <td>All-Rounder</td>
                <td>A mix of visitors like families, friends, and solo travelers. Prefers a balanced theme park experience with a variety of attractions.</td>
            </tr>
            <tr>
                <td>1</td>
                <td>Thrill-Seeking Socialite</td>
                <td>Dominated by young adults visiting with friends. Prefers roller coasters, shopping, and spontaneous exploration.</td>
            </tr>
            <tr>
                <td>2</td>
                <td>Tourist Explorer</td>
                <td>Composed entirely of tourists, with balanced preferences across attractions. Enjoys a mix of experiences and spontaneous navigation.</td>
            </tr>
            <tr>
                <td>3</td>
                <td>Family Fun-Seeker</td>
                <td>Predominantly families with young children and local visitors. Prefers food, general rides, and holiday visits.</td>
            </tr>
                <td>4</td>
                <td>Young Family Planner</td>
                <td>Most child-focused group, preferring structured planning, shows, and special events. Visits mainly on public holidays.</td>
            </tr>
            <tr>
                <td>5</td>
                <td>Budget-Conscious Adventurer</td>
                <td>Young adult locals who prioritizing cost, weather, and efficiency. Balances spontaneous exploration with queue management.</td>
            </tr>
            <tr>
                <td>6</td>
                <td>Event Enthusiast</td>
                <td>Primarily tourists who visit for special events and prioritize shows and standard rides. Enjoys weekdays and public holidays.</td>
            </tr>
        </tbody>
    </table>
</body>
